...
def eval(cfg, args, model, weights_basename, criterion, best_value):
    ...
    inf_step_arg = args.inf_step.lower()
    include_inf_steps = inf_step_arg != "false"
    noise_sweep_vals = args.sweep_vals
    inf_sweep_vals = args.num_inf_steps
    ...
    if include_inf_steps:
        inf_step_vals = inf_sweep_vals
    else:
        inf_step_vals = cfg.MODEL.UNROLLED.NUM_INF_STEPS
        
    values = itertools.product(
        cfg.DATASETS.TEST, cfg.AUG_TEST.UNDERSAMPLE.ACCELERATIONS, noise_vals, inf_step_vals
    )
    values = list(values)
    all_results = []
    ...
    for exp_idx, (dataset_name, acc, noise_level, num_inf_steps) in enumerate(values):
        ...
        model.num_inf_steps = num_inf_steps
        s_cfg.MODEL.UNROLLED.NUM_INF_STEPS = (num_inf_steps,)
        s_cfg.freeze()
        ...
        if metrics is not None:
            try:
                running_results = update_metrics(
                    all_results,
                    metrics,
                    on=["Acceleration", "dataset", "Noise Level", "Unrolled Steps", "weights", "Method", "rescaled"],
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="false",
        choices=("false", "true"),
        help="Whether or not to run inference on different number of unrolled steps",
    )
    parser.add_argument(
        "--num-inf-steps",
        default=[0],
        nargs="*",
        type=int,
        help="Number of unrolled iterations at inference",
    )
