...
class CommonMetricPrinter(EventWriter):
    ...
    def write(self):
        ...
        forward_time, data_time, time = None, None, None
        eta_string = "N/A"
        try:
        ...
        try:
            forward_time = storage.history("forward_time").global_avg() / 1000.0
            backward_time = storage.history("backward_time").global_avg() / 1000.0
        except KeyError: 
            pass
...
""".format(
                ...
                lr=lr+" forward_time: {:.4f}".format(forward_time) if forward_time is not None else "",
                memory="max_mem: {:.0f}M".format(max_mem_mb)+" backward_time: {:.4f}".format(backward_time) if max_mem_mb is not None else "",
