...
from ss_recon.engine import DGLTrainer, ADGLTrainer, hooks
...
from ss_recon.solver import GradAccumOptimizer, build_lr_scheduler, build_optimizer, build_greedy_optimizer
...
__all__ = ["GreedyTrainer","AsyncGreedyTrainer"]
...
class GreedyTrainer(DGLTrainer):
    """
    A trainer with DGL training logic. Compared to `SimpleTrainer`, it
    contains the following logic in addition:

    1. Separate network into modules, perform separate gradient updates for each module
    ...
    def __init__(self, cfg):
        ...
        optimizer_list = []
        if cfg.MODEL.UNROLLED.SHARE_WEIGHTS == False:
            for stage in range(cfg.MODEL.NUM_GREEDY_BLOCKS):
                optimizer_list.append(self.build_greedy_optimizer(cfg, model, stage))
        else:
            optimizer_list = cfg.MODEL.NUM_GREEDY_BLOCKS * [self.build_optimizer(cfg, model)]
        ...
        super().__init__(model, data_loader, optimizer_list, loss_computer)
        
        self.scheduler_list = []
        if cfg.MODEL.UNROLLED.SHARE_WEIGHTS == False:
            for num_schedulers in range(cfg.MODEL.NUM_GREEDY_BLOCKS):
                self.scheduler_list.append(self.build_lr_scheduler(cfg, optimizer_list[num_schedulers]))
        else:
            self.scheduler_list = cfg.MODEL.NUM_GREEDY_BLOCKS * [self.build_lr_scheduler(cfg, optimizer_list[0])]
    ...
    def build_hooks(self):
        ...
        ret = []
        for i in range(cfg.MODEL.NUM_GREEDY_BLOCKS):
                ret.append((
                    hooks.FlushOptimizer(self.optimizer_list[i], cfg.TEST.EVAL_PERIOD)
                    if isinstance(self.optimizer_list[i], GradAccumOptimizer)
                    else None
                ),)
                ret.append(hooks.IterationTimer())
                ret.append(hooks.LRScheduler(self.optimizer_list[i], self.scheduler_list[i]))
    ...
    def train(self):
        ...
        """
        super().train(self.start_iter, self.max_iter, self.cfg)
    ...
    @classmethod
    def build_greedy_optimizer(cls, cfg, model, stage):
        """
        Returns:
            torch.optim.Optimizer:
        ...
        Overwrite it if you'd like a different optimizer.
        """
        return build_greedy_optimizer(cfg, model, stage)
...
class AsyncGreedyTrainer(ADGLTrainer):
    """
    A trainer with ADGL training logic. Compared to `DGLTrainer`, it
    contains the following logic in addition:

    1. Train modules of a greedy model asynchronously
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.MULTIPROCESSING == True:
            num_modules = int(cfg.MODEL.UNROLLED.NUM_UNROLLED_STEPS/cfg.MODEL.NUM_PROCESSES)
            for k in range(cfg.MODEL.NUM_GREEDY_BLOCKS):  
                id = k % cfg.MODEL.NUM_PROCESSES
                model.resnets[k] = model.resnets[k].to(cfg.MODEL.DEVICE+":"+str(id))   

        else:
            model.to(cfg.MODEL.DEVICE)

        loss_computer = self.build_loss_computer(cfg)
        super().__init__(model, data_loader, optimizer_list, loss_computer, cfg)
