...
__all__ = ["MELModl3DUnrolledCNN"]

@META_ARCH_REGISTRY.register()
class MELModl3DUnrolledCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        num_mel_layers = num_reslayers*num_resblocks #Resblocks can only be 1 for MEL-MODL right now
        ...
        if share_weights:
            self.resnets = nn.ModuleList([RevNet(dims=3,num_channels=num_features,T=5,num_layers=num_mel_layers,device=self.device)] * num_grad_steps)
        else:
            self.resnets = nn.ModuleList([RevNet(dims=3,num_channels=num_features,T=5,num_layers=num_mel_layers,device=self.device) for _ in range(num_grad_steps)])
        ...
        if cfg.MODEL.TRAIN_MODE == "GRADCP":
            ...
            self.cpList = list(range(cfg.MODEL.NUM_CHECKPOINTS))
