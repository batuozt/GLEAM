...
__all__ = ["DGLModl2DUnrolledCNN"]


@META_ARCH_REGISTRY.register()
class DGLModl2DUnrolledCNN(nn.Module):
    """
    ...
    def forward(self, inputs, stage = 0, return_pp=False, vis_training=False, upto=False):
        ...
        device = self.resnets[stage].model.conv5.net[1].weight.device
        #device = self.device
...
class DGLUnrolledRep(nn.Module):
    ...
    def forward(self, image, stage, A, cg_solve, zf_image, dims, num_emaps,num_inf_steps,upto=False):
        ...
        out_image = out_image.permute(0, 2, 3, 1).reshape(dims[0:3] + (num_emaps, 2))
        if use_cplx:
