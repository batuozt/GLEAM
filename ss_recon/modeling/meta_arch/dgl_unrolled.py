...
__all__ = ["DGLGeneralizedUnrolledCNN"]


@META_ARCH_REGISTRY.register()
class DGLGeneralizedUnrolledCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        self.dgl_blocks = DGLUnrolledRep(self.resnets, self.step_sizes)
    ...
    def forward(self, inputs, stage = 0, return_pp=False, vis_training=False, upto=False):
        ...
        for dgl_stage in range(num_greedy_iters):
            ...
            image = self.dgl_blocks(image, cur_stage, A, zf_image, dims, self.num_emaps, self.num_inf_steps, upto)
...
class DGLUnrolledRep(nn.Module):
    def __init__(self, resnets, step_sizes):
        super(DGLUnrolledRep, self).__init__()
        self.resnets = resnets
        self.step_sizes = step_sizes
    def forward(self, image, stage, A, zf_image, dims, num_emaps,num_inf_steps,upto=False):
        ...
        if upto:
            ...
            for i in range(num_inf_steps):
                ...
                image = self.forward(image, i, A, zf_image, dims, num_emaps, num_inf_steps, upto=False)
            return image
        ...
        # prox update
        image = image.reshape(dims[0:3] + (num_emaps * 2,)).permute(0, 3, 1, 2)
        image = resnet(image)
        
        image = image.permute(0, 2, 3, 1).reshape(dims[0:3] + (num_emaps, 2))
        if use_cplx:
