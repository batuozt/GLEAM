...
def fft_forw(x, ndim=2):
    ndim = (2,3)
    data = torch.view_as_complex(x)
    data = ifftshift(data, dim=ndim)
    data = torch.fft.fftn(data, dim=ndim, norm="ortho")
    data = fftshift(data, dim=ndim)
    out = torch.view_as_real(data)
    return out
    #return torch.Tensor.fft(x, signal_ndim=ndim, normalized=True)
...
def fft_adj(x, ndim=2):
    ...
    data = torch.view_as_complex(x)
    data = ifftshift(data, dim=ndim)
    data = torch.fft.ifftn(data, dim=ndim, norm="ortho")
    data = fftshift(data, dim=ndim)
    out = torch.view_as_real(data)
