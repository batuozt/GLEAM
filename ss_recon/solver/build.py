...
def build_greedy_optimizer(cfg: CfgNode, model: torch.nn.Module, stage: int) -> torch.optim.Optimizer:
    """
    Build a greedy optimizer from config. Currently assumes meta architecture to be DGLGeneralizedUnrolledCNN.
    """
    params: List[Dict[str, Any]] = [] 
    num_greedy_blocks = cfg.MODEL.NUM_GREEDY_BLOCKS
    num_grad_steps = cfg.MODEL.UNROLLED.NUM_UNROLLED_STEPS
    num_greedy_iters = int(num_grad_steps/num_greedy_blocks)
    for i in range(num_greedy_iters):
        cur_id = stage * num_greedy_iters + i
        cur_block = model.resnets[cur_id]
        for key, value in cur_block.named_parameters():
            if not value.requires_grad:
                continue
            lr = cfg.SOLVER.BASE_LR
            weight_decay = cfg.SOLVER.WEIGHT_DECAY
            if key.endswith("norm.weight") or key.endswith("norm.bias"):
                ...
                weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
            params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
        #value = model.step_sizes[i]
        value = model.step_sizes[cur_id]
        params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
    optimizer = _build_opt(params, cfg)
    return optimizer
