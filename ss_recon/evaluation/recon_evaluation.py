...
class ReconEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        if self._skip_rescale:
            ...
            # Necessary to move target and preds to same device for ADGL Trainer
            if outputs["pred"].device != outputs["target"].device:
                outputs["target"] = outputs["target"].to(outputs["pred"].device)
    ...
    def evaluate_prediction(self, prediction, metric_names=None):
        ...
        if len(output.shape) > 4 and output.shape[0] > 1:
            output = output.view(output.shape[0]*output.shape[-2],output.shape[1],output.shape[2],1)
            target = target.view(target.shape[0]*target.shape[-2],output.shape[1],output.shape[2],1)
